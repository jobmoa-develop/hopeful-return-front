...
export default function UserManagementPage() {
  ...
  const handleResetPassword = async () => {
    ...
    if (
      !window.confirm(
        `${editTarget.name}(${editTarget.loginId}) 계정의 비밀번호를 초기화하시겠습니까?`,
      )
    ) {
      return;
    }
  ...
  return (
    <section className="view active" id="view-user-management">
      <div className="perm-bar">
        ...
        직원 관리 ·{' '}
        {canWrite ? '등록·수정·삭제 가능' : '조회만 가능 (등록/수정/삭제는 관리자 전용)'}
      </div>
              ...
              {isLoading ? (
                ...
                users.map((u) => (
                  <tr key={u.userId} onClick={() => void openEdit(u.userId)}>
                    <td className="pname" data-label="로그인 ID">
                      {u.loginId}
                    </td>
                    ...
                    {canWrite && (
                      ...
                      >
                        <button
                          className="btn tiny"
                          type="button"
                          onClick={() => void openEdit(u.userId)}
                        >
                          수정
                        </button>{' '}
                        <button
                      ...
                      <label className="chk" key={r.value}>
                        <input
                          ...
                          onChange={() =>
                            setCreateForm((f) => ({
                              ...f,
                              roleNames: toggleRole(f.roleNames, r.value),
                            }))
                          }
                        />
                        {r.label}
              ...
              <div className="form-grid">
                ...
                <div className="field">
                  ...
                  <input
                    ...
                    maxLength={50}
                    onChange={(e) =>
                      setEditForm((f) => (f ? { ...f, position: e.target.value } : f))
                    }
                  />
                </div>
                <div className="field">
                    ...
                    {ROLE_NAME_OPTIONS.map((r) => (
                      <label className="chk" key={r.value}>
                        <input
                          ...
                          onChange={() =>
                            setEditForm((f) =>
                              f ? { ...f, roleNames: toggleRole(f.roleNames, r.value) } : f,
                            )
                          }
                        />
                        {r.label}
                  ...
                  <div style={{ display: 'flex', gap: 16 }}>
                    ...
                    <label className="chk">
                      <input
                        ...
                        disabled={!canWrite}
                        onChange={(e) =>
                          setEditForm((f) => (f ? { ...f, locked: e.target.checked } : f))
                        }
                      />
                      잠금
                    </label>
              ...
              {canWrite && (
                <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px dashed var(--line)' }}>
                  ...
                  <p className="muted" style={{ fontSize: 12, margin: '4px 0 10px' }}>
                    사용자가 비밀번호를 잊었을 때 임시 비밀번호를 발급합니다. 발급 후 사용자에게
                    직접 전달해주세요.
                  </p>
                  {resetError && (
